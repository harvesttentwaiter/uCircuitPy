...
void sign(char *pub, char *secH, char *msg, char *out) {
	...
	EVP_PKEY *sec = EVP_PKEY_new_raw_private_key(EVP_PKEY_ED25519, NULL,
                                        private_key, 32 /*64*/);
	if (sec == NULL) printf("pkey sec null\n");
	//EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new_id(EVP_PKEY_ED25519, NULL);
	EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new(sec, NULL);
	if (pctx == NULL) printf("pkey ctx null\n");
	EVP_MD_CTX *ctx = EVP_MD_CTX_new();
	if (ctx == NULL) printf("md ctx null\n");
	int rv;
	EVP_MD_CTX_set_pkey_ctx(ctx, pctx);
	
	size_t sigSz=64;
	if (0) {
		rv = EVP_DigestSignInit(ctx, &pctx,
                        NULL /*EVP_MD *type*/, NULL, sec);
		if (rv != 1) printf("sig init fail %lu\n", ERR_get_error());                        
		rv = EVP_DigestSignUpdate(ctx, msg, strlen(msg));
		if (rv != 1) printf("update fail\n");
		rv = EVP_DigestSignFinal(ctx, signature, &sigSz);
		if (rv != 1) printf("final fail\n");
	} else {
		rv = EVP_DigestSign(ctx, signature, &sigSz, msg, strlen(msg));
		if (rv != 1) printf("sig fail\n");
	}
...
int verify(char *pubH, char *sig, char *msg) {
	...
	EVP_MD_CTX *ctx = EVP_MD_CTX_new();
	EVP_MD_CTX_set_pkey_ctx(ctx, pctx);
	/* sign 799f88d1a2703d1811816be54d3e2ae800dd8cc63edf66d1b3c5b355b13f066f b90988a1b0bdbb8625a5644c3b5d2140502520e6159b2de21e0e39c863969be8 Winky.19
begin main
sig a517b1a6e81eca859541bf9735a1da97f7a8d3e3ccc5c352996bf9c97d23cae2a5e01ca42ad5ea6c518e0e5e5e5d233b4cc06749cedfb69ff5efa9f598bcd605
end main
	*/
	size_t sigSz=64;
	rv = EVP_DigestVerify(ctx, signature, sigSz, msg, strlen(msg));
